...
# MAGIC 1. Download the binary version of H2O Sparkling Water at http://h2o-release.s3.amazonaws.com/sparkling-water/rel-2.3/3/index.html DO NOT USE MAVEN!
# MAGIC 2. Upload the Assembly jar `sparkling-water-assembly_2.11-2.3.3-all` library manually to your workspace and attach it to your cluster
# MAGIC 3. Create a new cluster using `DBR 4.1` (includes Apache Spark 2.3, Scala 2.11) and attach the Sparkling Water library (you must have `3` or more nodes)
...
options(rsparkling.sparklingwater.version = "2.3.3")
...
sc <- spark_connect(method = 'databricks', version = '2.3.0')
